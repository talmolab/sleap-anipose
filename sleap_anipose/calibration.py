...
def make_histogram(detections: np.ndarray, reprojections: np.ndarray, 
    ...
    if save:
        plt.savefig(Path(session) / "reprojection_errors.png", 
            format="png", dpi="figure")


def make_reproj_imgs(detections: np.ndarray, reprojections: np.ndarray, 
    frames: List[int], session: str = ".", n_samples=4, save: bool = False):
    ...
    for i, cam in enumerate(cam_folders):
        for frame in sampled_frames:
            ...
            plt.scatter(
                ...
                s=300, color='r', marker='+', label='detections', lw=2.5)
            plt.scatter(
                reprojections[i, frames.index(frame), :, 0],
                reprojections[i, frames.index(frame), :, 1],
                s=300, color='g', marker='x', label='reprojections', lw=2.5)
...
def get_metadata(
    corner_data: List[List[Dict]], cgroup: CameraGroup,
    save: bool = False, session: str = "."
    ...
    if save:
        ...
        with h5py.File(Path(session) / 'calibration.metadata.h5', 'w') as f:
            f.create_dataset(
                'frames', data=common_frames, chunks=True,
                compression='gzip', compression_opts=1
                )

            f.create_dataset(
                'detected_corners', data=common_corners, chunks=True,
                compression='gzip', compression_opts=1
                )

            f.create_dataset(
                'triangulated_corners', data=corners_3d, chunks=True,
                compression='gzip', compression_opts=1
                )

            f.create_dataset(
                'reprojected_corners', data=corners_reproj, chunks=True, 
                compression='gzip', compression_opts=1
                )
            ...
            f['detected_corners'].attrs[
                'Description'
                    ] = ('Shape: (n_cams, n_frames, n_corners, 2). '
                    f'View order: {cam_names}.')

            f['triangulated_corners'].attrs[
                'Description'
                    ] = ('Shape: (n_frames, n_corners, 3).')

            f['reprojected_corners'].attrs[
                'Description'
                    ] = ('Shape: (n_cams, n_corners, n_nodes, 2). '
                    f'View order: {cam_names}.')
...
def calibrate(
    session: str, board_width: int, board_height: int, square_length: float, 
    marker_length: float, marker_bits: int, dict_size: int, 
    save_calib: bool = False, save_metadata: bool = False, 
    histogram: bool = False, reproj_imgs: bool = False,
    save_hist: bool = False, save_reproj_imgs: bool = False
    ...
    """
    board = CharucoBoard(
        board_width, board_height, square_length,
    ...
    if reproj_imgs:
        make_reproj_imgs(detections, reprojections, frames, 
            session, save_reproj_imgs)
