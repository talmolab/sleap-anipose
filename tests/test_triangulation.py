...
def test_triangulate(minimal_session, tmp_path):
    ...
    save = ~(Path(minimal_session) / "points3d.h5").exists()

    if save:
        p3d = triangulate(minimal_session, cgroup, save=True, session=minimal_session)
        load_path = Path(minimal_session) / "points3d.h5"
    else:
    ...
    _, n_frames, n_tracks, n_nodes, _ = load_tracks(minimal_session)
...
def test_reproject(minimal_session, tmp_path):
